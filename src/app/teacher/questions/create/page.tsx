...
const DIFFICULTIES = ALL_DIFFICULTIES.map((d) => ({ value: d.id, label: d.label }));
...
function LatexPreview({ text }: { text: string }) {
  ...
  useEffect(() => {
    if (ref.current) ref.current.innerHTML = renderLatexPreview(text);
  }, [text]);
...
export default function CreateQuestionPage() {
  ...
  function setOptionContent(idx: number, content: string) {
    setForm((f) => ({
      ...f,
      options: f.options.map((o, i) => (i === idx ? { ...o, content } : o)),
    }));
  }
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <div className="flex items-center gap-4">
        ...
        <h1 className="text-2xl font-extrabold text-gray-900">Tạo câu hỏi mới</h1>
      </div>
          ...
          <CardBody className="space-y-4">
            ...
            <div>
              ...
              <textarea
                ...
                rows={4}
                placeholder="Nhập nội dung câu hỏi. Dùng $...$ cho LaTeX inline, $$...$$ cho display mode"
                value={form.content}
        ...
        <Card>
          ...
          <CardBody>
            <textarea
              className="w-full px-4 py-3 rounded-xl border-2 border-gray-200 focus:border-blue-500 focus:outline-none text-sm resize-none"
              rows={3}
              placeholder="Giải thích cách giải bài này..."
