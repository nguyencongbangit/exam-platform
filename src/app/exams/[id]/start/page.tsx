...
import { ChevronLeft, ChevronRight, Flag, Send, BookOpen, ShieldAlert, Maximize } from 'lucide-react';
...
function ExamStartContent({ examId }: { examId: string }) {
  ...
  const storageKey = `exam_${attemptId}`;
  const submittingRef = useRef(false);

  useEffect(() => {
    if (!attemptId) { router.push(`/exams/${examId}`); return; }
    loadAttempt();
  }, [attemptId]);

  // Fullscreen lock — only active after exam started
  useEffect(() => {
  ...
  async function enterFullscreenAndStart() {
    try {
      ...
      toast('Trình duyệt không hỗ trợ fullscreen tự động, vui lòng nhấn F11.', { icon: '⚠️' });
    }
  ...
  // Màn hình chờ — user phải click để vào fullscreen (bắt buộc bởi browser)
  if (!examStarted) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-900 via-blue-800 to-indigo-900 flex items-center justify-center p-4">
        <div className="bg-white rounded-3xl shadow-2xl max-w-md w-full p-8 text-center">
          ...
          <h1 className="text-2xl font-extrabold text-gray-900 mb-1">{attempt.exam.title}</h1>
          <p className="text-gray-500 text-sm mb-6">{attempt.exam.subject.name} · {attempt.exam.durationMinutes} phút · {totalQ} câu</p>

          <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-6 text-left text-sm text-amber-800 space-y-1.5">
            <p className="font-bold text-amber-900 flex items-center gap-2"><ShieldAlert size={15} /> Quy định làm bài:</p>
            <p>• Bài thi sẽ chạy <strong>toàn màn hình</strong>, không được thoát</p>
            <p>• Không được chuyển tab hoặc cửa sổ khác</p>
            <p>• Không được chuột phải</p>
            <p>• Vi phạm <strong>{MAX_VIOLATIONS} lần</strong> → bài tự động nộp</p>
          </div>

          <button
            onClick={enterFullscreenAndStart}
            className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-4 rounded-2xl text-lg flex items-center justify-center gap-3 transition-colors shadow-lg shadow-blue-200"
          >
            <Maximize size={22} />
            Vào toàn màn hình & Bắt đầu thi
          </button>
          <p className="text-xs text-gray-400 mt-3">Nhấn nút trên để xác nhận và bắt đầu tính giờ</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="exam-fullscreen" onContextMenu={(e) => e.preventDefault()}>
      ...
      {examStarted && !isFullscreen && !submittingRef.current && (
      ...
      <div className="shrink-0 bg-white border-b border-gray-100 shadow-sm">
        <div className="flex items-center justify-between px-4 h-14 max-w-full">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-xs text-gray-500 font-medium">{attempt.exam.subject.name}</div>
              <div className="text-sm font-bold text-gray-900 max-w-[200px] sm:max-w-md truncate">{attempt.exam.title}</div>
            </div>
          </div>
          <div className="flex items-center gap-3">
...
export default function ExamStartPage({ params }: { params: { id: string } }) {
  return (
    <Suspense fallback={<PageLoader />}>
      <ExamStartContent examId={params.id} />
    </Suspense>
  );
}
